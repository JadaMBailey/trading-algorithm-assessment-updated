    ...
    public Action evaluate(SimpleAlgoState state) {
        ...
        for (buy = 0; buy < state.getBidLevels(); buy++) // If less than order book bid size=true, then iterate variable
        {
          var checkingQuantity = state.getBidAt(buy).getQuantity();
            ...
            targetBidQuantity += checkingQuantity; // This accummulates the quanitity for bids on OrderBook
            logger.info("[MY-STRETCH-ALGO] In Algo Logic AND Checking sum of quantity " + targetBidQuantity); // Check for myself
        }
...
//                long price = (nearTouch.price - (spreadPrice / 2)); // Thinking if close to matching then position close to top
